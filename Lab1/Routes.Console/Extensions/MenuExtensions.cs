        ...
        do
        {
            ...
            switch (input)
            {
                ...
                case "3":
                    ...
                    var routes = routeRepository.GetRoutesWithTrolleybusesMoreThan(30);
                ...
                case "5":
                    ...
                    break;
                case "6":
                    Console.WriteLine("Отримати список номерів тролейбусів, що курсують на маршруті \u211610\n");
                    var trolleybuses = trolleybusRepository.GetTrolleybusesInRoute(10);
                    trolleybuses?.ForEach(x => stringBuilder.Append($"Number: {x}\n"));
                    Console.WriteLine(stringBuilder.ToString());
                    break;
                case "7":
                    Console.WriteLine(
                        "Отримати маршрути, де час проїзду менше ніж 20 хвилин і курсує понад 3 тролейбусів\n");
                    routes = routeRepository.GetRoutesWhereTravelTimeLessThanAndTrolleybusesMoreThan(20, 3);
                    routes?.ForEach(x => stringBuilder.Append($"{x.Name}|{x.Number}: {x.TravelTime} with {x.Trolleybuses?.Count}. Start: {x.StartStopEntity?.StopName}/End: {x.EndStopEntity?.StopName}\n"));
                    Console.WriteLine(stringBuilder.ToString());
                    break;
                case "8":
                ...
                case "9":
                    ...
                    var result = routeRepository.GetRoutesWhereTravelTimeMoreThanAndGroupedByTrolleybusesNumberOnRoute(30);
                ...
                case "18":
                    ...
                    result = routeRepository.GetRoutesWithTrolleybusesMoreThanAndAverageTravelTime(3);
                    Console.WriteLine(result);
                    break;
                case "19":
