...
public class RouteRepository(XDocument document) : IRouteRepository
{
    private readonly IEnumerable<XElement> _routes = document.Descendants("RouteEntity");
    ...
    {
        return _routes.Deserialize<RouteEntity>().Where(x => x.StartStopEntity?.StopName == stopName ||
    ...
    {
        return _routes.Deserialize<RouteEntity>()
    ...
    {
        return _routes.Deserialize<RouteEntity>().Where(x =>
    ...
    {
        return _routes.Deserialize<RouteEntity>().Where(x => x.TravelTime > minutes)
    ...
    {
        return _routes.Deserialize<RouteEntity>().Sum(x => x.TravelTime);
    }
}